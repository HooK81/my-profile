...
- **`auth/`** — JWT authentication with device-hash verification (passport-jwt). Token issued via `GET /v1/auth/token`. Device hash is `SHA-256(User-Agent)` — no shared secret, purely for browser-binding.
...
- **`api/`** — Axios client wrapper with automatic device-hash generation and token fetching.
