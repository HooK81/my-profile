...
describe('ProtectedText for Text', () => {
  ...
  it('Should ProtectedText render a text with customClassName', () => {
    const wrapper = mount(<ProtectedText text="foobar" customClassName="baz" />);
...
describe('ProtectedText for Link', () => {
  ...
  it('Should ProtectedText render a link with customClassName', () => {
    const wrapper = mount(<ProtectedText text="foobar" customClassName="baz" href="https://foobar.com"/>);
    expect(wrapper.find('span.protected-text.baz')).toHaveLength(1);
  });
  ...
  it('Should ProtectedText render a link with specific protectedHref', () => {
    const wrapper = mount(<ProtectedText text="foobar" href="https://foobar.com" protectedHref="https://do-not-click"/>);
    expect(wrapper.find('.protected-text a')).toHaveLength(1);
  ...
  it('Should ProtectedText can open Link with 1 header', () => {
    const wrapper = mount(<ProtectedText text="foobar" href="mailto:john@doe.com" hrefHeaders={{subject: "subject"}} />);
    expect(wrapper.find('.protected-text a')).toHaveLength(1);
    wrapper.find('.protected-text a').simulate('click');
  ...
  it('Should ProtectedText can open Link with more than 1 headers', () => {
    const wrapper = mount(<ProtectedText text="foobar" href="mailto:john@doe.com" hrefHeaders={{subject: "subject", cc: "foo@bar.com"}} />);
    expect(wrapper.find('.protected-text a')).toHaveLength(1);
    wrapper.find('.protected-text a').simulate('click');
    expect(window.location.assign).toHaveBeenCalledWith('mailto:john@doe.com?subject=subject&cc=foo%40bar.com');
  });
