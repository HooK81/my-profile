...
- **Mixins** (`_mixins.scss`): `respond-below()`, `respond-above()`, `respond-below-height()`, `container()`, `glass($blur)`, `primary-gradient()`, `icon-chip($size, $radius, $surface)`, `hover-lift` (lift + accent border + glow, used by Techs/Education/Hobbies), `glow-halo($size, $color)` (a `vmax`-sized circle with a 3-stop `color-mix` radial gradient, never `filter: blur` — an animated blur on a viewport-sized element freezes Safari; the animated Hero halos add `will-change: transform` so they are composited once)
