...
- **Glow blob** `glow-blob($size, $color, $blur)`: absolute circle, `filter: blur(120–150px)`, `pointer-events: none`, `z-index: -1` inside a Section (sections are `isolation: isolate`).
...
### Resume (`--bg-elevated`)
`--primary-soft-glow` blob 600px on the right (blur 140px). Column titles `h3` Sora 22px/600 with a 36px chip; 32px below; blocks 80px apart.
...
`--primary-glow` blob 700px bottom-left (blur 150px). Grid `minmax(0,1fr) minmax(0,1.4fr)` gap 24px, one column ≤ 768px. Info column: three glass rows `padding: 20px 22px`, radius 18px, 42px chip (`LuMapPin`, `LuSmartphone`, `LuMail`), `h4` 12px uppercase `.12em` `--muted` 600, value 500.
